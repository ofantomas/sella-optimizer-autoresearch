...
class Internals(BaseInternals):
    ...
    def find_all_bonds(
        self, nbond_cart_thr: int = 6, max_bonds: int = 20, scale: float = 1.25
    ) -> None:
    ...
    def guess_hessian(self, h0cart=67.5) -> np.ndarray:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if H0 is None:
            ...
            H0 = 1.0011958 * (P @ self.int.guess_hessian() @ P)
...
class MaxInternalStep(BaseRestrictedStep):
    ...
    def __init__(self, pes, *args, wx=1.0, wb=1.0, wa=1.0, wd=1.0, wo=1.0, **kwargs):
        if pes.int is None:
            raise ValueError(
...
_minimum_defaults = dict(
    delta0=0.1, sigma_inc=1.175, sigma_dec=0.9, rho_inc=1.035, rho_dec=120, method="qn"
)
    ...
    def step(self):
        ...
        if rho is None:
            ...
            if 1.0 / rho_band < rho < rho_band:
                ...
                self.delta = max(1.07298025 * smag, self.delta)
        self.rho = rho
        if self.rho is None:
            self.rho = 1.0
